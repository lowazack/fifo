...
export default {
  ...
  methods: {
    ...
    store() {
      let self = this;
      axios.post(this.$apiAdress + '/coreui/notes?token=' + localStorage.getItem("authtoken"),
          self.note
      )
  ...
  mounted: function () {
    let self = this;
    axios.get(this.$apiAdress + '/coreui/notes/create?token=' + localStorage.getItem("authtoken"))
        .then(function (response) {
