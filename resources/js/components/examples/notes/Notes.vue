...
export default {
  ...
  methods: {
    ...
    deleteNote ( id ) {
      let self = this;
      let noteId = id;
      axios.post(  this.$apiAdress + '/coreui/notes/' + id + '?token=' + localStorage.getItem("authtoken"), {
        _method: 'DELETE'
      })
    ...
    getNotes (){
      let self = this;
      axios.get(  this.$apiAdress + '/coreui/notes?token=' + localStorage.getItem("authtoken") )
      .then(function (response) {
