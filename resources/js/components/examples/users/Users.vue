...
export default {
  ...
  methods: {
    ...
    deleteUser ( id ) {
      let self = this;
      let userId = id;
      axios.post(  this.$apiAdress + '/coreui/users/' + id + '?token=' + localStorage.getItem("authtoken"), {
        _method: 'DELETE'
      })
    ...
    getUsers (){
      let self = this;
      axios.get(  this.$apiAdress + '/coreui/users?token=' + localStorage.getItem("authtoken"))
      .then(function (response) {
