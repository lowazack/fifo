...
export default {
  ...
  methods: {
    ...
    update() {
        ...
        axios.post(  this.$apiAdress + '/coreui/bread/' + self.$route.params.id + '?token=' + localStorage.getItem("authtoken"), postData )
  ...
  mounted: function(){
    let self = this
    axios.get(  this.$apiAdress + '/coreui/bread/' + self.$route.params.id + '/edit?token=' + localStorage.getItem("authtoken"))
    .then(function (response) {
