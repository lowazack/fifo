...
export default {
  ...
  methods: {
    ...
    deleteEmail ( id ) {
      let self = this;
      let noteId = id;
      axios.post(  this.$apiAdress + '/coreui/mail/' + id + '?token=' + localStorage.getItem("authtoken"), {
        _method: 'DELETE'
      })
    ...
    getTemplates (){
      let self = this;
      axios.get(  this.$apiAdress + '/coreui/mail?token=' + localStorage.getItem("authtoken") )
      .then(function (response) {
